...
20231122        mvh     Avoid .0 sequence in UIDs generated by #stage anonymization
...
static const void *MD5_body(MD5_CTX *ctx, const void *data, unsigned long size, bool correct)
{
	...
	do {
		...
		if (correct) {MD5_STEP(MD5_F, a, b, c, d, MD5_SET(0), 0xd76aa478, 7) }
...
void MD5_Update(MD5_CTX *ctx, const void *data, unsigned long size, bool correct)
{
	...
	if (used) {
		...
		size -= available;
		MD5_body(ctx, ctx->buffer, 64, correct);
	}

	if (size >= 64) {
		data = MD5_body(ctx, data, size & ~(unsigned long)0x3f, correct);
		size &= 0x3f;
	}
...
void MD5_Final(unsigned char *result, MD5_CTX *ctx, bool correct)
{
	unsigned long used, available;
...
void MD5_compute(char *in, unsigned char *out)
{
	MD5_CTX ctx;
	MD5_Init(&ctx);
	MD5_Update(&ctx, (unsigned char *)in, strlen(in), true);
	MD5_Final(out, &ctx, true);
}
...
ChangeUID(char *OldUID, const char *Type, char *NewUID, char *Stage, Database *db)
	...
	if (Stage && Stage[0]=='#')
		...
		MD5_Init(&ctx);
		MD5_Update(&ctx, dbUIDPrefix, strlen(dbUIDPrefix), false);
		MD5_Update(&ctx, OldUID, strlen(OldUID), false);
		MD5_Update(&ctx, Stage, strlen(Stage), false);
		MD5_Final(result, &ctx, false);
		...
		char *p;
		if ((p=strstr(NewUID+25, ".0"))) p[1]='1'; // replace .0 sequence if MD5 start with 0 with .1
		return TRUE;
		}
...
NewDeleteDICOM(
	...
	if (Thread) Progress.printf("Process=%d, Type='deletedicom', Active=1\n", Thread);
...
int	NewModifyDICOM(DICOMDataObject	*DDO, const char *script, int Thread, int copy)
	...
	if (Thread) Progress.printf("Process=%d, Type='modifydicom', Active=1\n", Thread);
	delete DDOCopy;
